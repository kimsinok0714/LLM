...
def load_vectorstore():
    ...
    if os.path.exists(VECTORSTORE_DIR):
        try:
            # FAISS 저장 시 pickle 파일로 문서 메타정보가 저장되기 때문에 이를 읽어와야 vectorstore가 완전하게 복원됨.
...
# Streamlit에서 새로고침하면 변수에 저장된 값이 모두 초기화됩니다.
# Streamlit은 새로고침해도 세션별 상태를 지속적으로 유지하기 위해 session_state를 사용합니다.
# 첫 실행에만 초기화하고 이후에는 값을 유지하기 위함
...
if vectordb_exists:
    ...
    if st.session_state.vectordb:
        ...
        # 업로드된 파일이 있다면 → 기존 벡터스토어에 추가
        if uploaded_file:
            with st.spinner("새 문서를 벡터스토어에 추가하는 중입니다..."):
                new_docs = load_and_split_docs(uploaded_file)
                st.session_state.vectordb.add_documents(new_docs)
                st.session_state.vectordb.save_local(VECTORSTORE_DIR)
                st.session_state.rag_chain = build_rag_chain(st.session_state.vectordb)
            st.success("새 문서를 기존 벡터스토어에 병합했습니다!")
            
    else:
        st.warning("벡터스토어를 불러오지 못했습니다. 새로 생성하세요.")

# 12. 벡터스토어가 없을 때: 업로드된 문서로 새로 벡터 스토어 생성
else:
    if uploaded_file:
